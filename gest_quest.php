...
while ($listeQuest = $Quest->fetch())
{
    ...
    <table class="quest">
        ...
        <tr>
            ...
            if (isset($_POST['suppr'.$listeQuest['QuestId'].''])) {
                $query = $co->prepare("DELETE FROM reponse WHERE RepQuestId = :Id ");
                $query->bindParam(':Id', $listeQuest['QuestId']);
                $query->execute();

                $query = $co->prepare("DELETE FROM question WHERE QuestId = :Id ");
                $query->bindParam(':Id', $listeQuest['QuestId'] );
                $query->execute();
    ...
    if (isset($_POST['suppr'.$listeRep['RepQuestId'].''])) {
        $query = $co->prepare("DELETE FROM reponse WHERE RepQuestId = :Id ");
        $query->bindParam(':Id', $listeRep['RepQuestId']);
        $query->execute();
        echo("<meta http-equiv='refresh' content='0.5'>");
    }
                        ?>
